...
def validateOptions(options):
    ...
    if options.prevJsonFile is not None and options.jsonFile is None:
        print(
            "It does not make sense to specify a previously used/analyzed JSON file without specifying a new JSON file, since with this option specified, the difference between the new and old JSON is taken as the lumi mask."
        )
        exit(-1)
...
def CheckProxy():
    proc = subprocess.Popen(
        ["voms-proxy-info", "--all"], stderr=subprocess.PIPE, stdout=subprocess.PIPE
    )
...
def checkStoragePath(storagePath):
    ...
    if len(storagePath) > 255:
        print
        print(
            "we might have a problem with output path lengths too long (if we want to run crab over these)."
        )
...
parser.add_option(
    "-r", "--run range", dest="runRange", help="selected run range", metavar="RUNRANGE"
)
...
with open(localInputListFile, "r") as f:
    for line in f:
        ...
        if "Summer16" in secondaryDatasetName or "Run2016" in secondaryDatasetName:
            ...
            print(
                'ERROR: could not determine year from secondaryDatasetName "{0}" from datasetName "{1}"'.format(
                    secondaryDatasetName, datasetName
                )
            )
            exit(-4)
        # get dataRun
        ...
        if options.jsonFile is not None and isData:
            if options.prevJsonFile is not None:
                print(
                    "Using the subtraction between previous json and new json; WARNING: if lumis changed from good in previous to bad in new json, this will not remove them"
                )
                from WMCore.DataStructs.LumiList import LumiList

                prevJsonLumiList = (
                    LumiList(url=options.prevJsonFile)
                    if "http:" in options.prevJsonFile
                    else LumiList(filename=options.prevJsonFile)
                )
                currentJsonLumiList = (
                    LumiList(url=options.jsonFile)
                    if "http:" in options.jsonFile
                    else LumiList(filename=options.jsonFile)
                )
